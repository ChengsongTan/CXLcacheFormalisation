theory HitSE_ModifiedStore
  imports "AllFixes.FixModifiedStore" HitSE_Defs
begin

lemma ModifiedStore'_HitSE_aux: assumes "SWMR_state_machine T \<and> CSTATE Modified T 0 \<and> nextStore T 0 "
...
lemma ModifiedStore'_SWHitSE_aux: assumes "SWMR_HitSE T \<and> CSTATE Modified T 0 \<and> nextStore T 0 "
  shows "SWMR_HitSE (T [ -=i 0])"
proof -
  have sh: "SWMR_HitSE T" using assms by (rule conjunct1)
  have g: "CSTATE Modified T 0 \<and> nextStore T 0" using assms by (rule conjunct2)
  have a: "SWMR_state_machine T \<and> CSTATE Modified T 0 \<and> nextStore T 0 "
    by (rule conjI[OF SWMR_HitSE_D1[OF sh] g])
  show ?thesis
    by (rule SWMR_HitSE_I[OF ModifiedStore'_coherent_aux_simpler[OF a] ModifiedStore'_HitSE_aux[OF a SWMR_HitSE_D2[OF sh]]])
qed

lemma ModifiedStore'_SWHitSE: shows "
SWMR_HitSE T \<Longrightarrow> Lall (ModifiedStore' T 0) SWMR_HitSE
"
unfolding ModifiedStore'_def clearBuffer_def
by (metis Lall.simps(1) Lall.simps(2) ModifiedStore'_SWHitSE_aux)

lemma ModifiedStore'_HitSE: "SWMR_state_machine T \<Longrightarrow> HitSE_inv T \<Longrightarrow> Lall (ModifiedStore' T 0) HitSE_inv"
  by (rule Lall_SWMR_HitSE_D2[OF ModifiedStore'_SWHitSE[OF SWMR_HitSE_I]])
