theory HitSE_MARspIFwdM
  imports "AllFixes.FixMARspIFwdM" HitSE_Defs
begin

lemma HostMARspIFwdM'_HitSE_aux: assumes "SWMR_state_machine T \<and> HSTATE MA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0 "
...
lemma HostMARspIFwdM'_SWHitSE_aux: assumes "SWMR_HitSE T \<and> HSTATE MA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0 "
...
proof -
  ...
  have g: "HSTATE MA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0" using assms by (rule conjunct2)
  have a: "SWMR_state_machine T \<and> HSTATE MA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0 "
    by (rule conjI[OF SWMR_HitSE_D1[OF sh] g])
  show ?thesis
    by (rule SWMR_HitSE_I[OF HostMARspIFwdM'_coherent_aux_simpler[OF a] HostMARspIFwdM'_HitSE_aux[OF a SWMR_HitSE_D2[OF sh]]])
qed

lemma HostMARspIFwdM'_SWHitSE: shows "
SWMR_HitSE T \<Longrightarrow> Lall (HostMARspIFwdM' T 0) SWMR_HitSE
"
unfolding HostMARspIFwdM'_def
unfolding sendGOFromSnpResp_def
apply(insert HostMARspIFwdM'_SWHitSE_aux)
by (smt (verit) helper)

lemma HostMARspIFwdM'_HitSE: "SWMR_state_machine T \<Longrightarrow> HitSE_inv T \<Longrightarrow> Lall (HostMARspIFwdM' T 0) HitSE_inv"
  by (rule Lall_SWMR_HitSE_D2[OF HostMARspIFwdM'_SWHitSE[OF SWMR_HitSE_I]])
