theory HitSE_SARspIFwdM
  imports "AllFixes.FixSARspIFwdM" HitSE_Defs
begin

lemma HostSARspIFwdM'_HitSE_aux: assumes "SWMR_state_machine T \<and> HSTATE SA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0 "
...
lemma HostSARspIFwdM'_SWHitSE_aux: assumes "SWMR_HitSE T \<and> HSTATE SA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0 "
  shows "SWMR_HitSE ( T [ 1 +=reqresp GO Shared txid] [ 5 sHost= SharedM] [ 0 -=snpresp  ])"
proof -
  have sh: "SWMR_HitSE T" using assms by (rule conjunct1)
  have g: "HSTATE SA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0" using assms by (rule conjunct2)
  have a: "SWMR_state_machine T \<and> HSTATE SA T \<and> nextSnpRespIs RspIFwdM T 0 \<and> GTS T 0 "
    by (rule conjI[OF SWMR_HitSE_D1[OF sh] g])
  show ?thesis
    by (rule SWMR_HitSE_I[OF HostSARspIFwdM'_coherent_aux_simpler[OF a] HostSARspIFwdM'_HitSE_aux[OF a SWMR_HitSE_D2[OF sh]]])
qed

lemma HostSARspIFwdM'_SWHitSE: shows "
SWMR_HitSE T \<Longrightarrow> Lall (HostSARspIFwdM' T 0) SWMR_HitSE
"
unfolding HostSARspIFwdM'_def
unfolding sendGOFromSnpResp_def
using HostSARspIFwdM'_SWHitSE_aux
by (smt (verit) Lall.simps(1) Lall.simps(2) add_0 clearBuffer_def one_mod_two_eq_one)

lemma HostSARspIFwdM'_HitSE: "SWMR_state_machine T \<Longrightarrow> HitSE_inv T \<Longrightarrow> Lall (HostSARspIFwdM' T 0) HitSE_inv"
  by (rule Lall_SWMR_HitSE_D2[OF HostSARspIFwdM'_SWHitSE[OF SWMR_HitSE_I]])
