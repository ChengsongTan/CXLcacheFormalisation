theory HitSE_SharedRdShared
  imports "AllFixes.FixSharedRdShared" HitSE_Defs
begin

lemma HostSharedRdShared'_HitSE_aux: assumes "SWMR_state_machine T \<and> HSTATE SharedM T \<and> nextReqIs RdShared T 0 \<and> GTS T 1 "
...
lemma HostSharedRdShared'_SWHitSE_aux: assumes "SWMR_HitSE T \<and> HSTATE SharedM T \<and> nextReqIs RdShared T 0 \<and> GTS T 1 "
...
proof -
  ...
  have g: "HSTATE SharedM T \<and> nextReqIs RdShared T 0 \<and> GTS T 1" using assms by (rule conjunct2)
  have a: "SWMR_state_machine T \<and> HSTATE SharedM T \<and> nextReqIs RdShared T 0 \<and> GTS T 1 "
    by (rule conjI[OF SWMR_HitSE_D1[OF sh] g])
  show ?thesis
    by (rule SWMR_HitSE_I[OF HostSharedRdShared'_coherent_aux_simpler[OF a] HostSharedRdShared'_HitSE_aux[OF a SWMR_HitSE_D2[OF sh]]])
qed

lemma HostSharedRdShared'_SWHitSE: shows "
SWMR_HitSE T \<Longrightarrow> Lall (HostSharedRdShared' T 0) SWMR_HitSE
"
unfolding HostSharedRdShared'_def sendHostDataGO_def
using HostSharedRdShared'_SWHitSE_aux
apply(insert HostSharedRdShared'_SWHitSE_aux)
by (smt (verit) helper)

lemma HostSharedRdShared'_HitSE: "SWMR_state_machine T \<Longrightarrow> HitSE_inv T \<Longrightarrow> Lall (HostSharedRdShared' T 0) HitSE_inv"
  by (rule Lall_SWMR_HitSE_D2[OF HostSharedRdShared'_SWHitSE[OF SWMR_HitSE_I]])
