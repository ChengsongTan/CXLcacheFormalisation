...
* **Model.** Two host rules in `BuggyRules.thy` changed. The conjunct `\<not> (CSTATE SIA T 1 \<and> nextGOPendingIs GO_WritePullDrop T 1)` was removed from the guard of `HostModifiedDirtyEvict'`; it used the fixed device index 1 in both instances of the rule. The guard of `HostMARspIHitSE'` is now `\<not> nextHTDDataPending T i` instead of `htddatas1 T = []`, which tested the H2D data channel of device index 0 in both instances of the rule. Two further changes answer DirtyEvicts that the model left unanswered: the conjunct `\<and> CSTATE IIA T i` was removed from the guard of `HostShared_DirtyEvict'` (its rule lemma never used it), and `HostModifiedDirtyEvictPrevious'`, already proved in `FixModifiedDirtyEvictPrevious.thy`, was added to `allTransitions'`, which now has 69 rules. The guard of `SIACGO'` no longer requires the host to be outside `SAD`, `MD` and `MA` (`saneSIACGO` in `BuggyRules.thy`), which the device cannot observe; two goals of its rule lemma now follow from invariant clauses that already cover those host states.
