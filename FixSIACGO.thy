...
theory FixSIACGO imports BasicInvariants begin
...
proof -
  have f1: "dthdatas1 ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) = dthdatas1 T" by (case_tac "program1 T", simp+)
  have f2: "nextGOPending T 0" using i2x by simp
  have f5: "HSTATE MD ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) = HSTATE MD T" by (metis HSTATE_rule_2)
  show ?thesis using i935 i1x f2 i3x f5 f1 by metis
qed
...
proof -
  have f2: "nextGOPending T 0" using i2x by simp
  have f3: "nextHTDDataPending ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) 1 = nextHTDDataPending T 1" by (case_tac "program1 T", simp+)
  have f4: "\<And>X. CSTATE X ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) 1 = CSTATE X T 1" by (metis CSTATE_SIACGO_otherside_invariant2)
  have f5: "HSTATE MA ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) = HSTATE MA T" by (metis HSTATE_rule_2)
  show ?thesis using i937 i1x f2 i3x f5 f3 f4 by metis
qed
